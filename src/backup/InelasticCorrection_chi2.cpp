...
TH2D* InelasticCorrection::performFitDxDy(
    ...
    auto objective = [&](const double *p) {
        ...
        for (const auto& bin : activeBins) {
            ...
            const double var = std::max(model, 1e-12);   // Pearson-like chi2
    ...
    minimizer->SetLimitedVariable(2, "rI",   0.1,  1e-3, 0.0,  0.42); // GEN3 0.42 atmost GEN4 0.70 GEN4b 0.75
    ...
    double& alpha, double& delta_inel, double Rnop, double& delta_qe, bool fit_shifts)
{
    ...
    minimizer->SetLimitedVariable(0, "alpha", 0.5, 1e-3, 0.0, 16.0);//GEN3 atmost 16 GEN4 35 GEN4b 22
    ...
    for(Long64_t i=0;i<ch_QE.GetEntries();++i){ 
        ...
        if (vQE.fnucl == 0) {
            if(std::strcmp(kin_, "GEN4_He3") == 0){
                hQE_dxdy_n.Fill(vQE.dy, dxq-0.12, vQE.weight);
            }
            else if(std::strcmp(kin_, "GEN4b_He3") == 0){
                hQE_dxdy_n.Fill(vQE.dy, dxq-0.05, vQE.weight);
            }
            else{
        ...
        if(vQE.fnucl == 0) {
            if(std::strcmp(kin_, "GEN3_He3") == 0){
                ...
                hQE_neutron.Fill(vQE.dx-0.03,vQE.weight);
            }
            else if(std::strcmp(kin_, "GEN4b_He3") == 0){
